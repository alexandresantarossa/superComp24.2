...
int preencherMochila(int capacidade, std::vector<Item>& itens, int& pesoAtual) {
    int valorMax = 0;
    pesoAtual = 0;

    for (const auto& item : itens) {
        if (item.peso + pesoAtual <= capacidade) {
            pesoAtual += item.peso;
            valorMax += item.valor;
        }
    }
...
int main() {
    ...
    for (const auto& caminhoArquivo : arquivosEntrada) {
        ...
        int valorMax = preencherMochila(capacidade, itens, pesoAtual);
